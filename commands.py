...
from volatility.renderers.csvForm import CSVRenderer
...
class Command(object):
    ...
    def render_csv(self, outfd, data):
        #attempts to executes CSVRenderer class and functions
        #Will produce errors if not possible
        try:
            self._render(outfd, CSVRenderer(), data)
        except NotImplementedError, why:
            debug.error(why) #prints generic error statement
        except TypeError, why:
            debug.error(why) #prints generic error statement
